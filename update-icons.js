    ...
    let updates = [];
        ...
        for (let key of actor.items.keys()) {
            let item = actor.items.get(key);
                ...
                let update = GetImageUpdate(actor, item);
                if (update !== null) {
                        updates.push(update);
                }
        }
...
function GetImageUpdate(actor, item) {
    ...
    let tokenImageName = actor.data.token && actor.data.token.img;
    let actorImageName = actor.data.img;

        if (imageName === null || imageName == "mystery-man.svg" || imageName == tokenImageName || imageName == actorImageName) {
